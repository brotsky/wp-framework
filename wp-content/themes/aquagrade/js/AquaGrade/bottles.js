...
$("#number-of-results").text($("#bottles-list .bottle").length);
$("#bottles-list").css("min-height",$(window).height());
$(window).resize(function(){
    $("#bottles-list").css("min-height",$(window).height());
});

function getBottles(data) {
    $( "#bottles-list" ).load( "?data=" + data['data'] + "&order=" + data['order'] + "&orderby=" + data['orderby'] + "&q=" + data['q'] + "&tdsmin=" + data['tdsmin'] + "&tdsmax=" + data['tdsmax'], function(){
        $("#number-of-results").text($("#bottles-list .bottle").length);
    } );
}

$("#bottle-q").on("change keyup", function(){
    data['q'] = $(this).val();
    setTimeout(function(){
        getBottles(data)
    }, 500);
...
$("#bottles-sidebar #orderby-options ul li a").click(function(e){
    e.preventDefault();
    
    $("#bottles-sidebar #orderby-options #orderby-button").html($(this).text() + " <span class='caret'></span>");
